...
func SearchThreatStraightFour(b board.Board, threat []Threat) []Threat{
	for y, row := range b {
		for x := range row {
			if (b.GetCell(board.Coords{x, y}) != 0) {
				...
				for i <= 4 {
					if x + 4 < 19 && x - 1 >= 0 {
						if (y - 1 >= 0 && y + 4 < 19 &&
							...
							(b.GetCell(board.Coords{x + 4, y + 4}) == 0) &&
						...
						if ((b.GetCell(board.Coords{x - 1, y}) == 0) &&
							(b.GetCell(board.Coords{x + 4, y}) == 0) &&
					...
					if y - 1 >= 0 && y + 4 < 19 {
						if ((b.GetCell(board.Coords{x, y - 1}) == 0) &&
							(b.GetCell(board.Coords{x, y + 4}) == 0) &&
				...
				if amountX == 4 {
					threat = append(threat, Threat{owner:b.GetCell(board.Coords{x, y}), positions:positionsX, rate:5})
				}
				if amountY == 4 {
