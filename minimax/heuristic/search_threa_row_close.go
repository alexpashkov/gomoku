		...
		for x := range row {
			if (b.GetCell(board.Coords{x, y}) != 0) {
				...
				for i <= len {
					...
					if y - 1 >= 0 && y + len < 19 {
						if (((b.GetCell(board.Coords{x, y - 1}) == 0 && b.GetCell(board.Coords{x, y + len}) != 0 && b.GetCell(board.Coords{x, y + len}) != current) ||
							(b.GetCell(board.Coords{x, y - 1}) != 0 && b.GetCell(board.Coords{x, y + len}) == 0 && b.GetCell(board.Coords{x, y - 1}) != current)) &&
							(b.GetCell(board.Coords{x, y + i}) == current)) {
							amountY++
							positionsY = append(positionsY, board.Coords{x, y + i})
						}
						if (x + 1 < 19 && x - len >= 0 &&
							((b.GetCell(board.Coords{x + 1, y - 1}) == 0 && b.GetCell(board.Coords{x - len, y + len}) != 0 && b.GetCell(board.Coords{x - len, y + len}) != current) ||
								(b.GetCell(board.Coords{x + 1, y - 1}) != 0 && b.GetCell(board.Coords{x - len, y + len}) == 0 && b.GetCell(board.Coords{x + 1, y - 1}) != current)) &&
							b.GetCell(board.Coords{x - i, y + i}) == current) {
							amountLeftZ++
				...
				if amountX == len {
					threat = append(threat, Threat{owner:current, positions:positionsX, size:int8(len), status:0})
				}
				if amountY == len {
					threat = append(threat, Threat{owner:current, positions:positionsY, size:int8(len), status:0})
				}
				if amountRightZ == len {
					threat = append(threat, Threat{owner:current, positions:positionsRightZ, size:int8(len), status:0})
				}
				if amountLeftZ == len {
					threat = append(threat, Threat{owner:current, positions:positionsLeftZ, size:int8(len), status:0})
				}
			}
		}
	}
